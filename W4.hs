...
isums :: Int -> IO Int
isums n = isums' [] n where
    isums' xs n = if length xs == n then return $ sum xs else do
        x <- readLn
        print $ sum (x:xs)
        isums' (x:xs) n
...
mkCounter = do
    r <- newIORef 0
    return (modifyIORef r (+1), readIORef r)
...
hFetchLines h nums = lineByLine 1 h nums where
    lineByLine _ h [] = do 
        hClose h
        return []
    lineByLine i h (n:ns)
        | not $ i == n = do
            hGetLine h
            lineByLine (i+1) h (n:ns)
        | otherwise = do
            isEnd <- hIsEOF h
            if isEnd then return [] else do
                this <- hGetLine h
                rest <- lineByLine (i+1) h ns
                return $ this : rest
...
--
-- NB! The lines might have different numbers of elemen*W4>*W4>*W4>*W4>*W4>*W4>ts.

readCSV :: FilePath -> IO [[String]]
readCSV path = do
    file <- readFile path
    return $ map split (lines file) where
...
compareFiles a b = do
    fileA <- readFile a
    fileB <- readFile b 
    mapM_ putStrLn $ diffLines (lines fileA) (lines fileB) where
        diffLines [] _ = []
        diffLines _ [] = []
        diffLines (x:xs) (y:ys) = if x /= y 
            then ("< " ++ x) : ("> " ++ y) : diffLines xs ys
            else diffLines xs ys
...
interact' f state = do
    str <- getLine
    case f (str,state) of
        (True, newStr, newState)  -> do
            putStr newStr
            interact' f newState
        (False, newStr, newState) -> do
            putStr newStr
            return newState
